...
class LeaguesDetialesViewController: UIViewController {
   ...
   override func viewDidLoad() {
    ...
    @IBAction func favoriteAction(_ sender: UIBarButtonItem) {
        if coreData.isLeagueInFavorate(ID: (myLeagues?.id)!) {
            favoriteButton.image = UIImage(systemName: "heart")
            coreData.deleteLeague(leagueID: (myLeagues?.id)!)
        }else{
            favoriteButton.image = UIImage(systemName: "heart.fill")
            coreData.addLeague(league: myLeagues!)
        }
    }
    
    func requestTeams(){
        if let leagueName = self.myLeagues?.name{
  ...
  func requestEvents(url:String,isUpcommingEvent:Bool){
        DispatchQueue.global(qos: .background).async {
            if let leagueId = self.myLeagues?.id{
